...
export default function GuidePage() {
  ...
  const executeCommand = useCallback((commandId: string) => {
    switch (commandId) {
      ...
      case "/stats":
      case "stats":
        showFeedback('info', 'Stats would appear here in workspace context');
        break;
      default:
          ...
          <div className="lg:col-span-1 space-y-2">
            ...
            {commandCategories.map((cat) => (
              ...
              >
                ...
                <span className="font-medium text-slate-900 dark:text-white">{cat.title}</span>
              ...
              <div className="space-y-4">
                {commandCategories
                  ...
                  ?.commands.map((cmd, idx) => (
                    <div
                      key={idx}
                      className="group flex items-start gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                    >
                      <button
                        onClick={() => executeCommand(cmd.command)}
                        className="px-3 py-1.5 bg-slate-900 dark:bg-slate-950 text-emerald-400 rounded-lg text-sm font-mono whitespace-nowrap hover:bg-slate-800 hover:text-emerald-300 transition-colors cursor-pointer"
                      ...
                      <button
                        onClick={() => executeCommand(cmd.command)}
                        className="opacity-0 group-hover:opacity-100 transition-opacity p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-lg hover:bg-blue-200"
