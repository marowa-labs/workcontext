...
const TOOL_ICONS: Record<string, React.ReactNode> = {
  slack: <MessageSquare className="w-6 h-6 text-purple-500" />,
  notion: <FileText className="w-6 h-6 text-gray-800 dark:text-gray-200" />,
  jira: <Ticket className="w-6 h-6 text-blue-500" />,
  github: <GitPullRequest className="w-6 h-6 text-gray-800 dark:text-gray-200" />,
  github_app: <Building2 className="w-6 h-6 text-gray-800 dark:text-gray-200" />,
  figma: <Palette className="w-6 h-6 text-pink-500" />,
};

const TOOL_COLORS: Record<string, string> = {
  slack: "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-800",
  notion: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700",
  jira: "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800",
  github: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700",
  github_app: "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700",
  figma: "bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-400 border-pink-200 dark:border-pink-800",
};
...
export default function IntegrationsPage() {
  ...
  const handleSync = async (connectionId: string) => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/integrations/${connectionId}/sync`,
  ...
  const fetchTree = useCallback(async (connectionId: string) => {
    try {
      setTreeLoading(true);
      const { data: { session } } = await supabase.auth.getSession();
      const token = session?.access_token;
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/integrations/${connectionId}/tree?${params}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const data = await res.json();
      if (data.success) {
  ...
  const handleImportTree = async (rootContentId: string) => {
    try {
      ...
      if (!token) return;
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/integrations/import-tree`,
        {
          method: "POST",
      ...
      const data = await res.json();
      if (data.success) {
        setSuccessMessage(`Imported "${data.title}" as a project (${data.source_tool}).`);
      } else {
  ...
  const handleDisconnect = async (connectionId: string, toolName: string) => {
    if (!confirm(`Disconnect ${toolName}? Existing synced content will be preserved.`)) return;
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const token = session?.access_token;
      if (!token) {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/integrations/${connectionId}`,
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001"}/api/integrations/search`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Connect your team&apos;s tools to unlock cross-platform semantic search.
          Search once, find answers from everywhere.
        </p>
      </div>
        ...
        {searchResults && searchResults.sources.length > 1 && (
          <div className="flex gap-2 mt-4 flex-wrap">
            ...
            {searchResults.sources.map((src) => (
              <button
                key={src.source}
                onClick={() => setActiveSourceFilter(activeSourceFilter === src.source ? null : src.source)}
                className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${
                          TOOL_COLORS[result.source] || "bg-gray-100 text-gray-700 border-gray-200"
                        }`}
                      >
                        {SOURCE_LABELS[result.source] || result.source}
                      </span>
                      <span className="text-xs text-muted-foreground">{result.content_type}</span>
      ...
      {/* Connected Tools */}
      <div>
        <h2 className="text-lg font-semibold text-foreground mb-4">Connected Tools</h2>
        {loading ? (
          ...
          <div className="bg-card border border-border rounded-xl p-8 text-center">
            ...
            <p className="text-muted-foreground">
              No tools connected yet. Connect your first tool below to start searching across your team&apos;s context.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {connections.map((conn) => (
              ...
              >
                <div className={`p-3 rounded-xl ${TOOL_COLORS[conn.tool_type] || "bg-gray-100 text-gray-700"}`}>
                  {TOOL_ICONS[conn.tool_type] || <Plug className="w-6 h-6" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-semibold text-foreground">{conn.display_name}</h3>
                    <span
                  ...
                  <p className="text-sm text-muted-foreground mt-0.5">
                    ...
                    {conn.last_synced_at && ` · Last synced ${formatDate(conn.last_synced_at)}`}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && fetchTree(browsingConnectionId)}
              ...
              <button
                onClick={() => { setBrowsingConnectionId(null); setTreeData([]); }}
                className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors"
              >
                <XCircle className="w-4 h-4" />
              </button>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-blue-900 dark:text-blue-300">
                        Import entire collection ({treeData[0].children.length} items)
                      </p>
                      <p className="text-xs text-blue-700 dark:text-blue-400">
                        Creates a single project with all content organized by type
                      </p>
                    </div>
                    <button
            ...
            {available.map((tool) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className={`p-2 rounded-lg ${TOOL_COLORS[tool.tool_type] || "bg-gray-100 text-gray-700"}`}>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-5">
        ...
        <ul className="text-sm text-blue-700 dark:text-blue-400 space-y-1">
          ...
          <li>2. We sync and index your content using AI embeddings for semantic understanding</li>
          <li>3. Search once across all tools — results are ranked by relevance with direct links to sources</li>
          <li>4. Import content into projects — Figma files render as live embeds, Notion pages as blocks</li>
          <li>5. To edit, click &quot;Edit in [Tool]&quot; — changes sync back when you re-sync</li>
          <li>6. Your data stays secure — tokens are encrypted, and we respect each tool&apos;s permissions</li>
        </ul>
      </div>
    </div>
  );
...
function TreeNode({ node, expandedNodes, toggleNode, onImport, onImportTree, depth }: TreeNodeProps) {
      ...
      >
        ...
        <button
          ...
          className={`w-5 h-5 flex items-center justify-center text-xs ${
            hasChildren ? "text-muted-foreground hover:text-foreground cursor-pointer" : "text-transparent cursor-default"
          }`}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          {node.author_name && (
            <span className="text-xs text-muted-foreground">by {node.author_name}</span>
          )}
        </div>
