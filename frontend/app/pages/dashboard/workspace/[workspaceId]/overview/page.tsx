...
export default function WorkspaceOverviewPage() {
  ...
  const currentUserMembership = ws?.members?.find(
            ...
            {(ws?.members ?? []).length === 0 ? (
              ...
              <ul className="space-y-2">
                {ws.members.slice(0, 8).map((m: any) => {
                  const u = m.user;
                  const isOwner = ws.owner_id === u.id;
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          {isOwner ? (
                            ...
                            <span
                              className={`text-[10px] font-medium px-1.5 py-0.5 rounded capitalize ${roleColors[m.role] ?? roleColors.viewer}`}
                            >
                              {m.role}
                            </span>
                          )}
