// Trigger a re-sync of the Figma connection (re-embeds all synced content)
...
(async () => {
  ...
  const conns = await c.query(
    "SELECT id, user_id, tool_type, status FROM external_tool_connections WHERE tool_type = 'figma' AND status = 'active'",
  );
  console.log("Active Figma connections:", conns.rows.length);

  if (conns.rows.length === 0) {
    console.log("No active Figma connection found.");
    await c.end();
    return;
  }
