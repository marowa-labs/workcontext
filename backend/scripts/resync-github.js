// Trigger a re-sync of the GitHub connection (re-embeds all synced content)
...
(async () => {
  ...
  const conns = await c.query(
    "SELECT id, user_id, tool_type, status FROM external_tool_connections WHERE tool_type = 'github' AND status = 'active'",
  );
  console.log("Active GitHub connections:", conns.rows.length);
